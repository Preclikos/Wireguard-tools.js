...
static std::map<std::string, WIREGUARD_ADAPTER_HANDLE> interfaces{};
...
std::string startAddon(const Napi::Env env)
{
  ...
  if (!WireGuardDll)
    return ((std::string) "Failed to initialize WireGuardNT, ").append(getErrorString(GetLastError()));
  ;
...
void listDevices::Execute()
{
  ...
  for (DWORD i = 0;; ++i)
  {
    ...
    if (!SetupDiGetDevicePropertyW(dev_info, &dev_info_data, &devpkey_name, &prop_type, (PBYTE)adapter_name, sizeof(adapter_name), NULL, 0) || prop_type != DEVPROP_TYPE_STRING)
      continue;
...
{
  WIREGUARD_ADAPTER_HANDLE Adapter = nullptr;
  if (interfaces.find(wgName) != interfaces.end())
  {
    Adapter = interfaces.find(wgName)->second;
  }
  if (!Adapter)
     return SetError("This interface not exists in Wireguard-Tools.js addon!");
   if (!(WireGuardSetAdapterState(Adapter, WIREGUARD_ADAPTER_STATE::WIREGUARD_ADAPTER_STATE_DOWN)))
     return SetError(std::string("Failed to set down interface, ").append(getErrorString(GetLastError())));
  interfaces.erase(wgName);
  WireGuardCloseAdapter(Adapter);
}
...
void getConfig::Execute()
{
  ...
  while (!(WireGuardGetConfiguration(Adapter, wg_iface, &buf_len)))
  {
    ...
    if (GetLastError() != ERROR_MORE_DATA)
      return SetError((std::string("Failed get interface config, code: ")).append(std::to_string(GetLastError())));
    wg_iface = (WIREGUARD_INTERFACE *)malloc(buf_len);
    if (!wg_iface)
      return SetError(((std::string) "Failed get interface config, ").append(std::to_string(-errno)));
  }

  if (wg_iface->Flags & WIREGUARD_INTERFACE_FLAG::WIREGUARD_INTERFACE_HAS_PRIVATE_KEY)
    privateKey = wgKeys::toString(wg_iface->PrivateKey);
  if (wg_iface->Flags & WIREGUARD_INTERFACE_FLAG::WIREGUARD_INTERFACE_HAS_PUBLIC_KEY)
    publicKey = wgKeys::toString(wg_iface->PublicKey);
  portListen = 0;
  if (wg_iface->Flags & WIREGUARD_INTERFACE_FLAG::WIREGUARD_INTERFACE_HAS_LISTEN_PORT)
  ...
  for (DWORD i = 0; i < wg_iface->PeersCount; i++)
  {
    ...
    if (wg_peer->Flags & WIREGUARD_PEER_FLAG::WIREGUARD_PEER_HAS_PERSISTENT_KEEPALIVE)
      peerConfig.keepInterval = wg_peer->PersistentKeepalive;
    if (wg_peer->LastHandshake > 0)
      peerConfig.last_handshake = (wg_peer->LastHandshake / 10000000 - 11644473600LL) * 1000;
...
void setConfig::Execute()
{
  ...
  if (interfaces.find(wgName) == interfaces.end())
  {
    Adapter = WireGuardCreateAdapter(toLpcwstr(wgName), L"Wireguard-tools.js", NULL);
       interfaces.insert(std::pair<std::string, WIREGUARD_ADAPTER_HANDLE>(wgName, Adapter));
  }
  else
  {
    Adapter = interfaces.find(wgName)->second;
  ...
  if (!Adapter)
    SetError(((std::string) "Failed to create adapter, ").append(getErrorString(GetLastError())));
