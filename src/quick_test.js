...
(0, node_test_1.default)("Wireguard quick config", async (t) => {
    await t.test("Stringify", async () => {
        const wgConfig = (0, quick_1.stringify)(StaticConfigJson);
        node_assert_1.default.strictEqual(wgConfig, StaticConfig);
    });
    await t.test("Parse", async () => {
        const wgConfig = (0, quick_1.parse)(StaticConfig);
        node_assert_1.default.deepEqual(wgConfig, StaticConfigJson);
    });
});
